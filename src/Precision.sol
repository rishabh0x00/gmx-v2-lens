...
import {Math as SafeMath} from "@openzeppelin/contracts/utils/math/Math.sol";
...
library Precision {
    ...
    function mulDiv(uint256 value, uint256 numerator, uint256 denominator) internal pure returns (uint256) {
        return SafeMath.mulDiv(value, numerator, denominator);
    }
    ...
    function mulDiv(uint256 value, uint256 numerator, uint256 denominator, bool roundUpMagnitude) internal pure returns (uint256) {
        if (roundUpMagnitude) {
            return SafeMath.mulDiv(value, numerator, denominator, SafeMath.Rounding.Ceil);
        }
    ...
    function toFactor(uint256 value, uint256 divisor, bool roundUpMagnitude) internal pure returns (uint256) {
        ...
        if (roundUpMagnitude) {
            return SafeMath.mulDiv(value, FLOAT_PRECISION, divisor, SafeMath.Rounding.Ceil);
        }

        return SafeMath.mulDiv(value, FLOAT_PRECISION, divisor);
    }
